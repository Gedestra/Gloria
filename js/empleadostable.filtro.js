...
function filterByValue(){
    let sucursal = document.getElementById("kt_select2_1").options[document.getElementById("kt_select2_1").selectedIndex].text
    let empleadosSucursal = [];
    if(sucursal != "todas"){
        contentTable.forEach(empleado => {
            if($('#filtroActividad').is(':checked')){
                if(empleado.sucursal == sucursal && empleado.estatus == 'Activo'){
                    empleadosSucursal.push(empleado);
                }
            }else{
...
function filterByActivity(){
    ...
    contentTable.forEach(empleado => {
        if(empleado.estatus == 'Activo'){
            empleadosActivos.push(empleado);
        }
    });
    printTable(empleadosActivos);
}
